...
## ⚙️ The Grounded Generation Pipeline (Under the Hood)

While this repository houses the front-end interactive testing client, the underlying question banks are generated via an automated, zero-hallucination pipeline:
...
[Primary & Secondary Sources] 
         ...
         ▼
 ┌───────────────┐
 │  NotebookLM   │  ◄── Enforces strict source grounding (direct quotes)
 └───────┬───────┘
         │ (Model Context Protocol - MCP)
         ▼
 ┌───────────────┐
 │ Agentic LLM   │  ◄── Generates formatted questions & rationales
 └───────┬───────┘
         │
         ▼
 ┌───────────────┐
 │ structured    │  ◄── Ingested into React app state (e.g., Unit 5 sample)
 │ questions.json│
 └───────────────┘
```

1. **The Hallucination Problem:** Generic LLMs often fabricate historical events or misinterpret curriculum standards when asked to generate AP-style questions.
2. **Model Context Protocol (MCP):** To guarantee factual integrity, the questions were generated by connecting a coding agent to a custom **Model Context Protocol (MCP) server** linked to **NotebookLM**.
3. **Source Grounding:** By feeding historical reading materials, unit notes, and primary sources directly into NotebookLM, the system was forced to ground its questions in direct quotes from the text, preventing hallucinated context.
4. **App Integration:** The pipeline generated a dataset of structured history questions, which were then compiled into the React application's local state (with the Unit 5 dataset serving as the live production build sample).
